...
def map_analysis(an: CodeAnalysis, *, artifact_id: str, data: bytes, run_id: str):
    ...
    def emit(predicate, subject_qn, object_qn, object_literal, locator, quoted):
        ...
        obj = by_qname.get(object_qn) if object_qn else None
        cid = claim_id(predicate=predicate, subject_id=subj, object_id=obj,
                       object_literal=object_literal,
                       extractor_id=an.backend_id, extractor_version=an.backend_version,
    ...
    for rr in an.references:
        ...
        object_qn = rr.to_qname if rr.resolution is Resolution.DETERMINISTIC else None
